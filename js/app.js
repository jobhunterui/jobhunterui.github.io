    ...
    if (window.db && user && window.firestoreExports && window.firestoreExports.onSnapshot) { //
        try {
            ...
            if (window.firestoreUnsubscribeUser[user.uid]) { //
                ...
                window.firestoreUnsubscribeUser[user.uid](); //
            }
            ...
            window.firestoreUnsubscribeUser[user.uid] = window.firestoreExports.onSnapshot( //
                ...
                (docSnapshot) => { 
                    if (docSnapshot.exists()) { //
                        ...
                        // Update UI with data from Firestore snapshot
                        // This might overwrite data fetched from /users/me if they differ,
                        // or complement it if /users/me is only called once on auth change.
                        window.currentUserSubscription = userDataFromFirestore.subscription || { tier: 'free', status: 'no_subscription_data' };
                        if (typeof window.updateUserSubscriptionUI === 'function') { //
                            window.updateUserSubscriptionUI(userDataFromFirestore.subscription); // Pass only the subscription part
                        }
                        if (typeof window.updateFeatureAccessUI === 'function') {
                             window.updateFeatureAccessUI(); // Re-evaluate feature access based on potentially new snapshot data
                        }

                    } else {
                        console.log("[Firestore] User document does not exist (onSnapshot). User might be new or data deleted."); //
                        if (typeof window.updateUserSubscriptionUI === 'function') { //
                            window.updateUserSubscriptionUI({ tier: 'free', status: 'active' }); // Explicitly pass an object that looks like a subscription
                        }
                         if (typeof window.updateFeatureAccessUI === 'function') {
                             window.updateFeatureAccessUI();
                        }
                    }
                },
                (error) => {
                    ...
                    if (typeof window.updateUserSubscriptionUI === 'function') { //
                        window.updateUserSubscriptionUI(null); 
                    }
                     if (typeof window.updateFeatureAccessUI === 'function') {
                         window.updateFeatureAccessUI();
                    }
